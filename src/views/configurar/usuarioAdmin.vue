<template>
    <section class="config-admin-admin">
        <p class="px-3 text-muted2 f-15 f-600 mb-5">Permiso de acceso entre Usuarios y Administradores</p>
        <div class="row permisos-listado px-5">
            <table class="tablapermisos tablapermisos-highlight-all w-100 mx-auto">
                <thead>
                <tr>
                    <th>Usuario / Admin</th>
                    <th v-for="(data,i) in array_usuarios" scope="col" v-if="[2,4].includes(data.id_rol)">
                        <!-- id_user_info -->
                ...
                <tbody>
                <tr v-for="(data,j) in array_usuarios"  v-if="data.id_rol === 3">
                    <!-- id_user_admin -->
                    <th scope="row">
                    ...
                    <td v-for="(data2,k) in array_usuarios" :key="k" class="text-center"  v-if="[2,4].includes(data2.id_rol)">
                        <el-checkbox
...
<script>
    ...
    export default {
        name: "permisoUsuarioAdmin",
        mounted(){
            this.listar();
        },
        computed:{
            ...mapGetters({
                array_usuarios:'configuracionPermisos/permisosUsuario_Admin',
            })
        },
        methods:{
            ...mapActions({
                listar:'configuracionPermisos/listarPermisosUser_Admin'
            }),
            set_check(admin,info,esActivo){
                var payload={admin:admin,info:info,esActivo:esActivo}
                this.$store.dispatch('configuracionPermisos/updatedPermisosUsuario_Admin',payload).then((data)=>{
            ...
            get_check(usuario,info){
                let array_temp = this.array_usuarios.find(a=>a.id === usuario)
                if (usuario==info) return true;
